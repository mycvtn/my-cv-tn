...
import { MapPin, Phone, Mail, Linkedin, Github, Award, Heart, Shield } from "lucide-react";
...
export const TunisianTemplate: React.FC<Props> = ({ data }) => {
  ...
  if (personalInfo.maritalStatus) {
    contactItems.push({
      id: "mar",
      icon: <Heart className={`w-3 h-3 text-slate-500 inline-block align-[-2px] ${isRTL ? "ml-1" : "mr-1"}`} />,
      ...
      <div className="pb-3 mb-3 border-b-2" style={{ borderColor: primaryColor }}>
        <div className="flex items-center justify-between gap-4">
          ...
          {settings?.showPhoto && personalInfo.photoUrl && (
            <div
              className="w-20 h-24 rounded border overflow-hidden flex-shrink-0 bg-slate-100 shadow-sm"
        ...
        <section className="mb-3 break-inside-avoid">
          ...
          <div className="flex flex-wrap gap-1 text-[9.5px] text-slate-800 pt-0.5">
            {skills.map((skill) => (
              <span
                key={skill.id}
                className="bg-rose-50/70 text-slate-800 px-1.5 py-0.5 rounded border border-rose-200/60 font-medium"
              >
                {skill.name}
              </span>
            ))}
